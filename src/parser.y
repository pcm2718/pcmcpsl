...
ProgramHead : OptConstDecls OptTypeDecls OptVarDecls PFDecls {cpslout << "program:" << std::endl;}
            ;
...
IfStatement : IfHead ThenPart ElseIfList ElseClause {emitFiLabel();} ENDSY
            ;

IfHead : IFSY Expression {emitIfControl($2); delete($2);}
       ;

ThenPart : THENSY StatementList
         ;

ElseIfList : ElseIfList {emitFiJump();} ElseIfHead ThenPart
           |
           ;
...
WhileStatement : WhileHead DOSY StatementList ENDSY {emitWhileJump(); emitWhileBottomLabel();}
               ;

WhileHead : WHILESY {emitWhileTopLabel();} Expression {emitWhileControl($3); delete($3);}
          ;
